...
from ritual_engine.npc_compiler import GuardianSQLOperations, GuardianDBTAdapter
...
from ritual_engine.npc_compiler import GuardianSQLOperations
...
class ModelRunner:
    ...
    def execute_model(self, model_name):
        ...
        if "synthesize(" in compiled_sql or "spread_and_sync(" in compiled_sql:
            print(f"Running Guardian operations for model: {model_name}")
...
def main():
    ...
    # Initialize the Guardian operations
    npcsql = GuardianSQLOperations(npc_directory="./npc_team", db_path=db_path)
