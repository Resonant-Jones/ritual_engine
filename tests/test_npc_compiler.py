...
from ritual_engine.npc_compiler import  Guardian, Jinx
...
def initialize_project(directory=None, context=None):
    """Initialize a new Guardian project"""
    ...
    # Create default Guardian (sibiji)
    ...
    with open(readme_path, "w") as f:
        f.write(f"""# {default_ctx["project_name"]} Guardian Team
        
Created: {default_ctx["created_at"]}

This directory contains Guardians, tools, and pipelines for the {default_ctx["project_name"]} project.

## Structure
- `.npc` files define AI agents with specific capabilities
- `tools/` directory contains tools that Guardians can use
...
# ---------------------------------------------------------------------------
# Example 2: Creating a Guardian Team
# ---------------------------------------------------------------------------

def example_2_npc_team():
    """Example demonstrating creation and use of a Guardian team"""
    print("\nExample 2: Creating and using a Guardian team")
    ...
    # Create specialist Guardians
    data_analyst = Guardian(
        "analyst",
...
def example_4_hierarchical_teams():
    ...
    # Sales department Guardians
    sales_manager = Guardian(
        "sales_manager",
    ...
    # Marketing department Guardians
    marketing_manager = Guardian(
        "marketing_manager",
    ...
    print("\nTeam structure:")
    print(f"Main team Guardians: {list(main_team.npcs.keys())}")
    for sub_team_name, sub_team in main_team.sub_teams.items():
        print(f"{sub_team_name} Guardians: {list(sub_team.npcs.keys())}")
...
"""
    ...
    # Create a Guardian with these tools
    language_npc = Guardian(
        "linguist",
