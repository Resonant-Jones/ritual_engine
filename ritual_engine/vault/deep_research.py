...
from ritual_engine.memory.knowledge_graph import *
import os

from ritual_engine.data import sample_primary_directives 

def generate_random_npcs(num_npcs, model, provider):
    """
    Function Description:
        This function generates a list of random Guardians.
    Args:
        num_npcs (int): The number of Guardians to generate.
    Returns:
        List[Guardian]: A list of generated Guardians.
    """
    # Initialize the list of Guardians
    npcs = []
    
    # Generate the Guardians
    for i, primary_directive in np.random.choice(sample_primary_directives, num_npcs):
        npc = Guardian(primary_directive=primary_directive, 
                    model=model, 
                    provider=provider,)
...
def generate_research_chain(request, npc, depth, memory=5, context=None):
    """
    Function Description:
        This function generates a research chain for the given Guardian.
    Args:
        npc (Guardian): The Guardian for which to generate the research chain.
