...
from ritual_engine.npc_sysenv import get_locally_available_models
from ritual_engine.vault.command_history import (
    CommandHistory,
    save_conversation_message,
)
from ritual_engine.npc_compiler import  Jinx, Guardian

from ritual_engine.codex import (
...
def get_project_npc_directory(current_path=None):
    """
    Get the project Guardian directory based on the current path
    
    Args:
        current_path: The current path where project Guardians should be looked for
...
def load_npc_by_name_and_source(name, source, db_conn=None, current_path=None):
    """
    Loads a Guardian from either project or global directory based on source
    
    Args:
        name: The name of the Guardian to load
        source: Either 'project' or 'global' indicating where to look for the Guardian
        db_conn: Optional database connection
        current_path: The current path where project Guardians should be looked for
    
    Returns:
        Guardian object or None if not found
    """
    if not db_conn:
    ...
    if source == 'project':
        ...
        print(f"Looking for project Guardian in: {npc_directory}")
    else:  # Default to global if not specified or unknown
        npc_directory = user_npc_directory
        print(f"Looking for global Guardian in: {npc_directory}")
    
    # Look for the Guardian file in the appropriate directory
    npc_path = os.path.join(npc_directory, f"{name}.npc")
    
    if os.path.exists(npc_path):
        try:
            npc = Guardian(file=npc_path, db_conn=db_conn)
            return npc
        except Exception as e:
            print(f"Error loading Guardian {name} from {source}: {str(e)}")
            return None
    else:
        print(f"Guardian file not found: {npc_path}")
        return None
...
def capture():
    # Capture screenshot using Guardian-based method
        ...
        if os.path.exists(npcshrc_path):
            with open(npcshrc_path, "r") as f:
                for line in f:
                    ...
                    key_mapping = {
                        "GuardianSH_MODEL": "model",
                        "GuardianSH_PROVIDER": "provider",
                        "GuardianSH_EMBEDDING_MODEL": "embedding_model",
                        "GuardianSH_EMBEDDING_PROVIDER": "embedding_provider",
                        "GuardianSH_SEARCH_PROVIDER": "search_provider",
                        "GuardianSH_LICENSE_KEY": "GuardianSH_LICENSE_KEY",
                        "GuardianSH_STREAM_OUTPUT": "GuardianSH_STREAM_OUTPUT",
                        "GUARDIAN_STUDIO_DEFAULT_FOLDER": "default_folder",
                    }
...
def save_global_settings():
    ...
    try:
        ...
        key_mapping = {
            "model": "GuardianSH_CHAT_MODEL",
            "provider": "GuardianSH_CHAT_PROVIDER",
            "embedding_model": "GuardianSH_EMBEDDING_MODEL",
            "embedding_provider": "GuardianSH_EMBEDDING_PROVIDER",
            "search_provider": "GuardianSH_SEARCH_PROVIDER",
            "GuardianSH_LICENSE_KEY": "GuardianSH_LICENSE_KEY",
            "GuardianSH_STREAM_OUTPUT": "GuardianSH_STREAM_OUTPUT",
            "default_folder": "GUARDIAN_STUDIO_DEFAULT_FOLDER",
        }
...
@app.route("/api/guardian_team_global")
def get_guardian_team_global():
    try:
        db_conn = get_db_connection()
        ...
        # Use existing helper to get Guardians from the global directory
        for file in os.listdir(global_npc_directory):
            if file.endswith(".npc"):
                npc_path = os.path.join(global_npc_directory, file)
                npc = Guardian(file=npc_path, db_conn=db_conn)

                # Serialize the Guardian data - updated for the new Jinx structure
                serialized_guardian = {
                    "name": npc.name,
                ...
                guardian_data.append(serialized_guardian)

        return jsonify({"guardians": guardian_data, "error": None})

    except Exception as e:
        print(f"Error loading global Guardians: {str(e)}")
        return jsonify({"guardians": [], "error": str(e)})
...
@app.route("/api/save_guardian", methods=["POST"])
def save_guardian():
    try:
        data = request.json
        ...
        if not guardian_data or "name" not in guardian_data:
            return jsonify({"error": "Invalid Guardian data"}), 400
        ...
        yaml_content = f"""name: {guardian_data['name']}
primary_directive: "{guardian_data['primary_directive']}"
model: {guardian_data['model']}
provider: {guardian_data['provider']}
api_url: {guardian_data.get('api_url', '')}
use_global_jinxs: {str(guardian_data.get('use_global_jinxs', True)).lower()}
"""

        # Save the file
        file_path = os.path.join(npc_directory, f"{guardian_data['name']}.npc")
        with open(file_path, "w") as f:
            f.write(yaml_content)

        return jsonify({"message": "Guardian saved successfully", "error": None})

    except Exception as e:
        print(f"Error saving Guardian: {str(e)}")
        return jsonify({"error": str(e)}), 500


@app.route("/api/guardian_team_project", methods=["GET"])
def get_guardian_team_project():
    try:
        db_conn = get_db_connection()
        ...
        for file in os.listdir(project_npc_directory):
            ...
            if file.endswith(".npc"):
                ...
                # Serialize the Guardian data, updated for new Jinx structure
                serialized_guardian = {
                    "name": npc.name,
                    "primary_directive": npc.primary_directive,
                    "model": npc.model,
        ...
        print(f"Error fetching Guardian team: {str(e)}")
        return jsonify({"guardians": [], "error": str(e)})
...
def get_attachment_response():
    ...
    guardian_name = data.get("guardian")
    guardian_source = data.get("guardianSource", "global")
    team = data.get("team")
    ...
    # Load the Guardian if a name was provided
    guardian_object = None
    if guardian_name:
        db_conn = get_db_connection()
        # Pass the current_path parameter when loading project Guardians
        guardian_object = load_npc_by_name_and_source(guardian_name, guardian_source, db_conn, current_path)
        
        if not guardian_object and guardian_source == 'project':
            # Try global as fallback
            print(f"Guardian {guardian_name} not found in project directory, trying global...")
            guardian_object = load_npc_by_name_and_source(guardian_name, 'global', db_conn)
            
        if guardian_object:
            print(f"Successfully loaded Guardian {guardian_name} from {guardian_source} directory")
        else:
            print(f"Warning: Could not load Guardian {guardian_name}")
    ...
    response = get_llm_response(
        ...
        messages=messages,
        model=model,
        npc=guardian_object,  # Pass the Guardian object instead of just the name
    )
    ...
    save_conversation_message(
        ...
        model=model, provider=provider, npc=guardian, attachments=attachments
...
def stream():
    ...
    guardian_name = data.get("guardian", None)
    guardian_source = data.get("guardianSource", "global")
    ...
    if npc_name:
        ...
        if not npc_object and npc_source == 'project':
            print(f"Guardian {npc_name} not found in project directory, trying global...")
            npc_object = load_npc_by_name_and_source(npc_name, 'global', db_conn)
        if npc_object:
            print(f"Successfully loaded Guardian {npc_name} from {npc_source} directory")
        else:
            print(f"Warning: Could not load Guardian {npc_name}")
