<p align="center">
  <img src="https://raw.githubusercontent.com/Resonan-Jones/ritual_engine/main/ritual_engine.png" alt="ritual_engine logo of a solarpunk sign">
</p>


# ritual_engine

Welcome to `ritual_engine`, the python library for the NPC Toolkit that supercharges natural language processing pipelines and agent tooling. `ritual_engine` is a python framework for building systems with LLMs that can generate text, images, and videos while allowing users to easily integrate data sources in their response handling.


<p align="center">
  <a href= "https://github.com/Resonan-Jones/ritual_engine/blob/main/docs/ritual_engine.md"> 
  <img src="" ></a>
</p>
...
```
from ritual_engine.npc_compiler import NPC, Team
ggm = NPC(
...
```
The structure of ritual_engine also allows one to pass an npc
...
`ritual_engine` also supports streaming responses, with the `response` key containing a generator in such cases which can be printed and processed through the print_and_process_stream method.
...
```
Return structured outputs by specifying `format='json'` or passing a Pydantic schema. When specific formats are extracted, `ritual_engine`'s `get_llm_response` will convert the response from its string representation so you don't have to worry about that. 
...
```
from ritual_engine.llm_funcs import gen_image
...
```
from ritual_engine.llm_funcs import gen_video
...
For more examples of how to use `ritual_engine` to simplify your LLM workflows  or to create agents or multi-agent systems, see [here](https://github.com/Resonant_Jones/ritual_engine). `ritual_engine` can include images, pdfs, and csvs in its llm response generation. 


## Inference Capabilities
- `ritual_engine` works with local and enterprise LLM providers through its LiteLLM integration, allowing users to run inference from Ollama, LMStudio, OpenAI, Anthropic, Gemini, and Deepseek, making it a versatile tool for both simple commands and sophisticated AI-driven tasks. 

## Read the Docs

Read the docs at [ritual_engine.readthedocs.io](https://ritual_engine.readthedocs.io/en/latest/)
...
## Mailing List
Interested to stay in the loop and to hear the latest and greatest about `ritual_engine`, `npcsh` and NPC Studio? Be sure to sign up for the [newsletter](resonantconstructs.ai/newsletter!



## Support
If you appreciate the work here, [consider supporting NPC Worldwide with a monthly donation](), [buying NPC-WW themed merch](), or hiring us to help you explore how to use `ritual_engine` and AI tools to help your business or research team, please reach out to info@resonantconstructs.ai .
...
- `ritual_engine` is a framework that speeds up and simplifies the development of NLP-based or Agent-based applications and provides developers and researchers with methods to explore and test across dozens of models, providers, and personas as well as other model-level hyperparameters (e.g. `temperature`, `top_k`, etc.), incorporating an array of data sources and common tools.
- The `ritual_engine` agent data layer makes it easy to set up teams and serve them so you can focus more on the agent personas and less on the nitty gritty of inference.
- `ritual_engine` provides pioneering methods in the construction and updating of knowledge graphs as well as in the development and testing of novel mixture of agent scenarios.
- In `ritual_engine`, all agentic capabilities are developed and tested using small local models (like `llama3.2`, `gemma3n`) to ensure it can function reliably at the edge of computing.
...
For cases where you wish to set up a team of NPCs, jinxs, and assembly lines, add a `Guardian_team` directory to your project and then initialize an NPC Team.
```bash
